...
def solve_recursion(grid, row, col):
    # Проверяем дошли ли до последнего столбца, если не дошли ищем подходящее значение значение, если дошли проверяем ряд
    if col == 9:
        ...
        row += 1
        col = 0
    if grid[row][col] > 0:  # если в текущей позиции не 0 значение  увеличиваем глубину рекурсии с увеличением столбца
    ...
    for number in range(1, 10):
        if check_valid(sudoku_grid, row, col, number):
            ...
            grid[row][col] = number
            #  увеличиваем глубину рекурсии с увеличением столбца,  если возвращается True мы прошли все поле
            if solve_recursion(sudoku_grid, row, col + 1):
                return True  # возвращение из рекурсии bactrack
...
def check_valid(grid, row, col, number):
    ...
    sud_square_col = col - col % 3  # находим координаты квадрата
    for i in range(3):
        for j in range(3):
